...
public class Main extends JavaPlugin implements Listener {
        ...
        if (command.getName().equalsIgnoreCase("hibernate") &&
            ...
            if (args.length == 1) {
                if (args[0].equalsIgnoreCase("reload")) {
                    reloadConfig();
                    loadConfiguration();
                    log.sendMessage(ColorUtils.getColored("&a[Hibernate] Configuration reloaded!"));
                    return true;
                }
                if (args[0].equalsIgnoreCase("status")) {
                    sender.sendMessage(String.format(ColorUtils.getColored("&a[Hibernate] Hibernation is currently &%s%s"),
                        hibernateConfig.enabled ? "a" : "c",
                        hibernateConfig.enabled ? "enabled" : "disabled"));
                    sender.sendMessage(String.format(ColorUtils.getColored("&a[Hibernate] Deep hibernation is &%s%s"),
                        hibernateConfig.deepHibernation ? "a" : "c",
                        hibernateConfig.deepHibernation ? "enabled" : "disabled"));
                    sender.sendMessage(String.format(ColorUtils.getColored("&a[Hibernate] Hibernation countdown: &e%d seconds"),
                    ...
                    sender.sendMessage(String.format(ColorUtils.getColored("&a[Hibernate] The server is currently &%s%s"), color, serverState));
                    return true;
                }
                if (args[0].equalsIgnoreCase("wake")) {
                    if (countdownActive) {
                        ...
                        countdownTimer = 0;
                        log.sendMessage(ColorUtils.getColored("&a[Hibernate] Hibernation countdown cancelled!"));
                    } else if (isDeepHibernating || isHibernating) {
                        wakeUpServer();
                        log.sendMessage(ColorUtils.getColored("&a[Hibernate] Server has been manually awakened!"));
                    } else {
                        log.sendMessage(ColorUtils.getColored("&c[Hibernate] Server is already awake and no countdown is active!"));
                    }
                    return true;
                }
                if (args[0].equalsIgnoreCase("sleep")) {
                    if (!isDeepHibernating && !isHibernating) {
                        ...
                        log.sendMessage(ColorUtils.getColored("&a[Hibernate] Server has been manually put to sleep!"));
                    } else {
                        log.sendMessage(ColorUtils.getColored("&c[Hibernate] Server is already hibernating!"));
                    }
                    return true;
                }
            } else {
                log.sendMessage(ColorUtils.getColored("&c[Hibernate] Usage: /hibernate <reload|status|wake|sleep>"));
                return true;
            }
        }
        return false;
    ...
    public void onEnable() {
        ...
        if (foundBlacklistedPlugins.isEmpty()) {
            ...
            log.sendMessage(ColorUtils.getColored("&c[Hibernate] Blacklisted plugins were found (plugins that will stop working if Hibernate is enabled). " +
        ...
        Bukkit.getScheduler().scheduleSyncRepeatingTask(this, new Runnable() {
            ...
            public void run() {
                ...
                if (onlineCount > 0 && countdownActive) {
                    countdownActive = false;
                    countdownTimer = 0;
                    log.sendMessage(ColorUtils.getColored("&6[Hibernate] Hibernation countdown cancelled - players joined!"));
                    return;
                }
                ...
                if (onlineCount == 0 && hibernateConfig.enabled) {
                    if (hibernateConfig.playerCheckCount != 0) {
                        ...
                        log.sendMessage(ColorUtils.getColored("&6[Hibernate] Starting hibernation countdown: " + countdownTimer + " seconds"));
                        return;
                    }
                    ...
                    if (countdownActive) {
                        ...
                        if (countdownTimer == 20 || countdownTimer == 10 || countdownTimer <= 5 && countdownTimer > 0) {
                            Bukkit.broadcastMessage(ColorUtils.getColored("&6[Hibernate] Server will hibernate in " + countdownTimer + " seconds..."));
                        }

                        if (countdownTimer <= 0) {
                            countdownActive = false;
                            log.sendMessage(ColorUtils.getColored("&6[Hibernate] Hibernation countdown completed - No players online, entering hibernation mode"));
    ...
    private void unloadAllChunksAggressively() {
        ...
        log.sendMessage(ColorUtils.getColored("&a[Hibernate] Deep hibernation: Aggressively unloaded " + totalUnloaded + " chunks"));
    }
    ...
    private void wakeUpServer() {
        ...
        if (countdownActive) {
            countdownActive = false;
            countdownTimer = 0;
            log.sendMessage(ColorUtils.getColored("&6[Hibernate] Hibernation countdown cancelled due to server wake up"));
        }
    ...
    private void performGarbageCollection() {
        ...
        if (memoryFreed > 0L) {
            log.sendMessage(ColorUtils.getColored(String.format("&a[Hibernate] %d MB memory freed using Java garbage collector", memoryFreed)));
        }
    }
}